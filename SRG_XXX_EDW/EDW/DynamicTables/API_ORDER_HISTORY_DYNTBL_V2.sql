...
2024-01-11  Raymond Zhao    performance tunning
*****************************************************************/
...
(
    select vbrp.vkorg_auft as SALES_ORG,
        ...
        case when charindex('CS',vbrk.xblnr,1)>0 then 'CSR'
            ...
            -- when nullif(vbrp.shkzg,'') is null and SalesNum >= 0 then 'Sales' -- not required due to next when condition
    ...
    from sapecc.vbrp_0 vbrp 
        ...
        inner join sapecc.likp_0 likp on vbrp.vgbel=likp.vbeln
        inner join sapecc.vbak_0 vbak on vbrp.aubel=vbak.vbeln
        inner join sapecc.vbuk_0 vbuk on vbrp.vbeln=vbuk.vbeln and vbrk.vtweg = 'S3'
    where vbrk.fkdat::int >= 20140101 -- int instead of date for performance
        -- filter on Sales/Returns transactions - using subset of TransactionIndicator logic
        and
        (
            nullif(vbrp.shkzg,'') is null -- Sales: don't filter on SalesNum to include cancelled items too
            or vbrk.fkart = 'RE' -- Returns
            or SalesNum < 0 and vbrp.shkzg = 'X' -- 'Returns'
        )
)
group by 
    CUSTOMER,
